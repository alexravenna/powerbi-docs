...
| Filtering | No. You can't filter or slice a dashboard. You can filter a [dashboard tile in focus mode](../explore-reports/end-user-focus.md#work-in-focus-mode), but can't save the filter. | Yes. There are many different ways to filter, highlight, and slice. |
